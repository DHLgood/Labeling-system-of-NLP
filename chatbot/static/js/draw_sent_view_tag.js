...
model_show = function (MODEL_LIDT_ID, model_list) {
    var parentContainer = document.getElementById(MODEL_LIDT_ID);

    //初始化ul：删除所有的子元素，进行初始化
    while (parentContainer.childNodes.length > 0)
        parentContainer.removeChild(parentContainer.lastChild);
    deEle = document.createElement("OPTION");
    deEle.setAttribute("value", '分词');
    deEle.setAttribute("selected","selected");
    deEle.textContent = '分词';
    parentContainer.appendChild(deEle)

    for (var i =0; i < model_list.length; i++){
        opEle = document.createElement("OPTION");
        opEle.setAttribute("value", model_list[i]);
        opEle.textContent = model_list[i];
        parentContainer.appendChild(opEle)
    }


}

model_tag_show = function (returnModelTagRst){
    var parentContainer_one = document.body.querySelector("#label-explanation-panel-one");
    var parentContainer_two = document.body.querySelector("#label-explanation-panel-two");
    $(".label-label-primary").remove();

    for (var i =0; i < returnModelTagRst.length; i++){

        opEle = document.createElement("DIV");
        opEle.setAttribute("class", "label-label-primary");
        opEle.setAttribute("id", returnModelTagRst[i]);
        opEle.setAttribute("style", "font-size: 2.4em;cursor: pointer");
        opEle.setAttribute("draggable", "true");
        opEle.textContent = returnModelTagRst[i];
        if (i%2 == 0) {
            parentContainer_one.firstElementChild.appendChild(opEle);
            }
        else {
            parentContainer_two.firstElementChild.appendChild(opEle);
            }
    }
...
export_data = function (EXPORT_DATA, model_list) {
    var parentContainer = document.getElementById(EXPORT_DATA);
    ...
    deEle = document.createElement("OPTION");
    deEle.setAttribute("value", '请选择标注类型');
    deEle.setAttribute("selected","selected");
    deEle.textContent = '请选择标注类型';
    parentContainer.appendChild(deEle)

    deEle = document.createElement("OPTION");
    deEle.setAttribute("value", '全部类型');
    deEle.textContent = '全部类型';
    parentContainer.appendChild(deEle)
...
data_source_show= function (EXPORT_DATA_SOURCE, model_list) {
    var parentContainer = document.getElementById(EXPORT_DATA_SOURCE);

    //初始化ul：删除所有的子元素，进行初始化
    while (parentContainer.childNodes.length > 0)
        parentContainer.removeChild(parentContainer.lastChild);

    deEleAll = document.createElement("OPTION");
    deEleAll.setAttribute("value", '全部类型');
    deEleAll.textContent = '全部类型';
    parentContainer.appendChild(deEleAll)

    deEleAll = document.createElement("OPTION");
    deEleAll.setAttribute("value", '请选择数据来源');
    deEleAll.setAttribute("selected","selected");
    deEleAll.textContent = '请选择数据来源';
    parentContainer.appendChild(deEleAll)
...
super_export_data = function (EXPORT_DATA, model_list) {
    var parentContainer = document.getElementById(EXPORT_DATA);
...
//    parentContainer.appendChild(deEle)
    ...
    deEle = document.createElement("OPTION");
    deEle.setAttribute("value", '请选择标志类型');
    deEle.setAttribute("selected","selected");
    deEle.textContent = '请选择标志类型';
    parentContainer.appendChild(deEle)
...
super_data_source_show= function (EXPORT_DATA_SOURCE, model_list) {
    var parentContainer = document.getElementById(EXPORT_DATA_SOURCE);
